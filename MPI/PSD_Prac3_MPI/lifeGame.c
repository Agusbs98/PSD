...
int main(int argc, char* argv[]){
	...
	if (rank == MASTER){		
		...
		for(int i = 1; i<= totalIterations;++i){
			...
			if(i%2 == 0){
				if(distModeStatic){
					masterStatic(worldB,worldA,worldWidth,worldHeight, size, renderer);
				}
				else{
					masterDinamic(worldB,worldA,worldWidth,worldHeight, grainSize, size, renderer);
				}
				clearWorld(worldB, worldWidth,worldHeight);
			}
			else{
				...
				if(distModeStatic){
					masterStatic(worldA,worldB,worldWidth,worldHeight, size, renderer);
				}
				else{
					masterDinamic(worldA,worldB,worldWidth,worldHeight, grainSize, size, renderer);
				}
				clearWorld(worldA, worldWidth,worldHeight);
			}
			SDL_RenderPresent(renderer);
...
// mpiexec -hostfile machines -np 5 ./lifeGame 100 100 10 auto salida.bmp dynamic 10 
